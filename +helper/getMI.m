function AMI_ = getMI(true_label,predicted)
if sum(true_label== 0)
...
if nargin==1
    T=true_label; %contingency table pre-supplied
elseif nargin==2
    %build the contingency table from membership arrays
    R=max(true_label);
    C=max(predicted);
    n=length(predicted);N=n;
    %identify & removing the missing labels
    list_t=ismember(1:R,true_label);
    list_m=ismember(1:C,predicted);
    T=Contingency(true_label,predicted);
    T=T(list_t,list_m);
end
